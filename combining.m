...
columns_of_interest = [1, 3, 4, 5, 6, 7, 8, 9]; 
...
for i = 1:length(fields)
    ...
    % Extract the data and store it in the cell array
    data_cell{i} = all_data.(fields{i}).organoid_data;
    
    % Extract the regionprops data for the current field
    regionprops_data = all_data.(fields{i}).nuclei_data;
    ...
    for j = 1:numel(regionprops_data)
        ...
        temp_mean_values_cell{j} = mean(table_data(:, 1:end-1), 1);
...
new_column_names_combined_mean = strcat('Nuclei_', combined_mean_values.Properties.VariableNames(1:6));
combined_mean_values.Properties.VariableNames = [new_column_names_combined_mean, 'Organoid_Lumen_Classification'];
new_column_names_combined_nuclei = strcat('Nuclei_', combined_nuclei_data.Properties.VariableNames(1:6));
combined_nuclei_data.Properties.VariableNames = [new_column_names_combined_nuclei, 'Organoid_Lumen_Classification', combined_nuclei_data.Properties.VariableNames{8}];

% Join the two tables
average_table = horzcat(combined_organoid_data(:, 1:6), combined_mean_values(:, 1:7));
