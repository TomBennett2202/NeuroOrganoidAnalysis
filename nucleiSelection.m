...
for i = 1:numel(organoid_mask_files)
    ...
    % Display the image again and allow the user to manually select non-mitotic nuclei
    figure;
    imshow(current_image(:,:,3));
    hold on;
    title('Click on non-mitotic nuclei, then press Enter.', 'FontSize', 15);
    
    [x_non_mitotic, y_non_mitotic] = getpts; % Get points selected by the user
    
    % Initialize an array to store the indices of non-mitotic nuclei
    non_mitotic_nuclei_indices = [];
    
    % Loop through the clicked points and determine the corresponding nuclei
    for j = 1:numel(x_non_mitotic)
        clicked_point = [x_non_mitotic(j), y_non_mitotic(j)];
        % Get nucleus index for the point
        non_mitotic_nucleus_idx = nuclei_mask(round(clicked_point(2)), round(clicked_point(1)));
        % Check it is not background
        if non_mitotic_nucleus_idx > 0
            non_mitotic_nuclei_indices = [non_mitotic_nuclei_indices; non_mitotic_nucleus_idx];
        end
    end
